...
Web uses the official `libghostty-vt` C ABI for parsing, terminal state, grapheme
...
- The artifact is built from the revision in `native/libghostty-vt/VERSION`.

The web adapter owns browser font shaping, the hidden IME textarea, clipboard and DOM input,
and its Canvas renderer. It also delegates application mouse encoding, word and line
...
Update the revision in `native/libghostty-vt/VERSION`, where the upstream `LICENSE`
also lives. Then run:
...
compares it against the canonical `VERSION` — so the web vendor directory holds only the artifacts, drift
