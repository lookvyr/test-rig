...
In Test Rig Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Test Rig uses Claude Code's normal config directory.

When you set this field, Test Rig points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Test Rig looks.

Then add another Claude provider in Test Rig:
...
Test Rig only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Test Rig keeps separate config directories isolated
...
Add or edit a Claude provider in Test Rig Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Test Rig stores the value as a server secret and does not
...
Test Rig does not need a special Claude Code Router provider. Treat the router as a Claude
