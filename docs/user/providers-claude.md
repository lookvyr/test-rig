...
This guide is for people who want to use more than one Claude setup in Sightseer. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Sightseer](./install.md).
...
In Sightseer Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Sightseer uses Claude Code's normal config directory.

When you set this field, Sightseer points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Sightseer looks.

Then add another Claude provider in Sightseer:
...
Sightseer only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Sightseer keeps separate config directories isolated
...
Add or edit a Claude provider in Sightseer Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Sightseer stores the value as a server secret and does not
...
Sightseer does not need a special Claude Code Router provider. Treat the router as a Claude
