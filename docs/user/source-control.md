...
Use **Show changed files** to search paths and navigate the changed-file tree. The tree stays beside the diff in a wide panel and opens over it in a narrow panel. Drag its left edge to adjust its width; your preferred width is remembered. Arrow keys browse files; clicking a file or pressing Enter opens it and keeps the tree open at any panel width. Use **Hide changed files** to close the tree. **Previous changed file** and **Next changed file** move back and forth between diffs in either stacked or split view. Selecting a collapsed file expands it.
