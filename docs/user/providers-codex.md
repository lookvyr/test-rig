...
This guide is for people who want to use more than one Codex account in Sightseer. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Sightseer](./install.md).
...
In Sightseer Settings, name it something obvious:
...
In Sightseer Settings, add another Codex provider:
...
Sightseer shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Sightseer treats it
