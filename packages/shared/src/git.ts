...
import { NewWorktreeBranchPrefix } from "@t3tools/contracts/settings";
...
const TEMPORARY_WORKTREE_BRANCH_SEGMENT = "_worktree";
// Legacy canonical form was `t3code/<8 hex>`. Older mobile builds generated `t3code/<uuid>`
...
const LEGACY_TEMP_WORKTREE_BRANCH_PATTERN = new RegExp(
  `^${LEGACY_WORKTREE_BRANCH_PREFIX}\\/(?:[0-9a-f]{8}|[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$`,
);
const TEMP_WORKTREE_BRANCH_PATTERN = new RegExp(
  `^(.+)\\/${TEMPORARY_WORKTREE_BRANCH_SEGMENT}\\/([0-9a-f]{8})$`,
);
...
export function buildTemporaryWorktreeBranchName(
  ...
  const token = randomHex(4)
    ...
    .replace(/[^0-9a-f]/g, "")
    .slice(0, 8);
  return `${prefix}/${TEMPORARY_WORKTREE_BRANCH_SEGMENT}/${token}`;
}

export function parseTemporaryWorktreeBranchPrefix(refName: string): string | null {
  const trimmed = refName.trim();
  if (LEGACY_TEMP_WORKTREE_BRANCH_PATTERN.test(trimmed.toLowerCase())) {
...
export function isTemporaryWorktreeBranch(refName: string): boolean {
  return parseTemporaryWorktreeBranchPrefix(refName) !== null;
}
