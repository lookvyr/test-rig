...
if command -v sightseer >/dev/null 2>&1; then
  exec sightseer "$@"
fi
...
printf 'Remote host is missing the Sightseer CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.sightseer/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.sightseer"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-sightseer.sh"
RUNNER_NEXT="$STATE_DIR/run-sightseer.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Sightseer server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.sightseer/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.sightseer"
RUNNER_FILE="$STATE_DIR/run-sightseer.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@lookvyr/sightseer@latest");
