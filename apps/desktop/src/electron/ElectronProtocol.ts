...
export function makeDesktopContentSecurityPolicy(input: DesktopProtocolRegistrationInput): string {
  ...
  // User-configured environment origins are not known when this response
  // policy is created, so restrict connections by the network schemes the
  // client supports instead of by host.
