...
function ChatViewContent(props: ChatViewProps) {
  ...
  const threadVisitTimestamp = getThreadVisitTimestamp(serverThread);
  useEffect(() => {
    if (!serverThread?.id || !threadVisitTimestamp) return;
    markThreadVisited(
      scopedThreadKey(scopeThreadRef(serverThread.environmentId, serverThread.id)),
      threadVisitTimestamp,
    );
  }, [markThreadVisited, serverThread?.environmentId, serverThread?.id, threadVisitTimestamp]);
  useEffect(() => {
  ...
  const systemComposerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
    ...
    if (activeThread && showVersionMismatchBanner && versionMismatch && versionMismatchDismissKey) {
      items.push({
        ...
        description: versionMismatchGuidance(versionMismatchServerLabel),
  ...
  const {
    ...
    composerRef,
    promptRef,
    onRespond: (requestId, answers) => onRespondToUserInput(requestId, answers),
  });
  ...
  const persistThreadSettingsForNextTurn = usePersistThreadSettings(environmentId, serverThread);
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      const sideChatFocused =
        (event.target instanceof Element && event.target.closest("[data-side-chat]") !== null) ||
        document.querySelector("[data-side-chat] [data-composer-model-picker-open]") !== null;
      ...
      if (command === "modelPicker.toggle" && !sideChatFocused) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (command === "thread.stop") {
        if (!canInterruptRunningThread || sideChatFocused) return;
        event.preventDefault();
        event.stopPropagation();
    ...
    activeRightPanelSurface?.kind === "side-chat" ? (
      ...
      <Suspense fallback={null}>
        <DiffPanel
          ...
          mode="embedded"
          composerDraftTarget={sideDiffThreadRef ?? composerDraftTarget}
