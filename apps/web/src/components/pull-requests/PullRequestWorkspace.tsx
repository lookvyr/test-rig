...
function ProjectPullRequests({
  ...
  const { filters, selection, panelOpen, select, clearSelection } = usePullRequestQueueStore();
  const query = useEnvironmentQuery(
    gitEnvironment.pullRequests({
      environmentId: project.environmentId,
      input: { cwd: project.workspaceRoot, state: filters.state, involvement: filters.involvement },
    }),
  );
      ...
      {items.map((pr) => {
        ...
        return (
          <button
            ...
            className="block w-full border-b px-5 py-4 text-left hover:bg-accent/40 aria-pressed:bg-accent/60 focus-visible:outline-2 focus-visible:outline-ring"
          >
    ...
    <SidebarInset className="h-dvh min-h-0 overflow-hidden bg-background">
      ...
      <div className="relative flex min-h-0 flex-1 overflow-hidden">
        ...
        {panelOpen && selection && (
          <aside
            className={cn(
              "flex min-h-0 w-full min-w-0 flex-col border-l md:w-[min(55%,48rem)]",
              panelExpanded && "md:w-full",
            )}
          >
            {selectionAvailable ? (
