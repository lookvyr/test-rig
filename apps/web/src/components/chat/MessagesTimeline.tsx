...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const [minimapAtEnd, setMinimapAtEnd] = useState(false);
  ...
  const handleScroll = useCallback(() => {
    ...
    const nextCurrentIndex = resolveTimelineMinimapCurrentIndex({
      scrollTop,
      itemTops: itemBounds.map((item) => item.top),
    });
    setMinimapAtEnd(state.isAtEnd === true);
          ...
          <TimelineMinimap
            ...
            onSelect={(item) => {
              ...
              void listRef.current?.scrollToIndex({
                index: item.rowIndex,
                animated: true,
                viewOffset: TIMELINE_TURN_SCROLL_OFFSET,
              });
            }}
          />
        </div>
...
function TimelineMinimap({
  ...
  const nextItem = isAtEnd ? null : (items[(resolvedCurrentIndex ?? -1) + 1] ?? null);
