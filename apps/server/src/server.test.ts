...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("does not expose removed Connect HTTP or DPoP authentication paths", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();

      const removedRouteUrl = yield* getHttpServerUrl("/api/connect/relay-config");
      const removedRouteResponse = yield* fetchEffect(removedRouteUrl, { method: "POST" });
      assert.equal(removedRouteResponse.status, 404);

      const websocketTicketUrl = yield* getHttpServerUrl("/api/auth/websocket-ticket");
      const legacyDpopResponse = yield* fetchEffect(websocketTicketUrl, {
        method: "POST",
        headers: { authorization: "DPoP historical-session-token" },
      });
      assert.equal(legacyDpopResponse.status, 401);
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  for (const desktopOrigin of ["sightseer://app", "sightseer-dev://app"]) {
  ...
  it.effect("stores browser OTLP traces locally without forwarding them", () =>
    Effect.gen(function* () {
  ...
  it.effect("routes websocket rpc subscribeServerConfig streams snapshot then update", () =>
    Effect.gen(function* () {
      ...
      if (first?.type === "snapshot") {
        ...
        assert.isUndefined(first.config.observability.otlpTracesUrl);
        assert.equal(first.config.observability.otlpTracesEnabled, false);
        assert.isUndefined(first.config.observability.otlpMetricsUrl);
