...
import { PRODUCT_PACKAGE_NAME } from "@t3tools/shared/productIdentity";

/**
 * A pinned runtime is an exact `@lookvyr/sightseer@<version>` npm install under
 ...
 * switching over, never `npx sightseer`, whose cache is ephemeral and whose
...
/**
 * Installs `@lookvyr/sightseer@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned Sightseer runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
          ...
          "--no-fund",
          "--no-audit",
          `${PRODUCT_PACKAGE_NAME}@${input.version}`,
        ],
