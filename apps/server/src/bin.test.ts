...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("does not expose inherited service commands", () =>
    Effect.gen(function* () {
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["sightseer", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["sightseer", "project", "add"]);
