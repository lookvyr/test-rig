...
export interface AuthenticatedSession {
  readonly sessionId: AuthSessionId;
  readonly subject: string;
  readonly method: Exclude<ServerAuthSessionMethod, "dpop-access-token">;
  readonly scopes: ReadonlyArray<AuthEnvironmentScope>;
...
function acceptSupportedSession(
  session: SessionStore.VerifiedSession,
): Effect.Effect<AuthenticatedSession, ServerAuthInvalidCredentialError> {
  if (session.method === "dpop-access-token" || session.proofKeyThumbprint !== undefined) {
    return Effect.fail(
      new ServerAuthInvalidCredentialError({
        diagnostic: "Managed relay sessions are no longer supported.",
      }),
    );
  }
...
export const make = Effect.gen(function* () {
  ...
  > =>
    sessions.verify(token).pipe(
      ...
      Effect.flatMap(acceptSupportedSession),
    );
      ...
      if (Option.isSome(requestUrl)) {
        ...
        if (websocketTicket && websocketTicket.trim().length > 0) {
          return yield* sessions
            ...
            .pipe(mapSessionVerificationErrors, Effect.flatMap(acceptSupportedSession));
        }
      }
