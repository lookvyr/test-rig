...
import { PRODUCT_PACKAGE_NAME, PRODUCT_SLUG } from "@t3tools/shared/productIdentity";
 ...
 * Detection is best-effort; callers must fail closed to a plain `sightseer` command.
 */
...
/**
 * The Sightseer package spec to suggest. The literal spec the user typed is
 * resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? `${PRODUCT_PACKAGE_NAME}@nightly` : PRODUCT_PACKAGE_NAME;
}

/**
 * Render a `sightseer <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `${PRODUCT_SLUG} ${input.subcommand}`;
  }
