...
const parentFixture: OrchestrationThread = {
  id: parentId,
  projectId,
  title: "Parent task",
...
for (const providerName of ["codex", "claudeAgent"] as const) {
  it.layer(NodeServices.layer)(`side chat lifecycle decisions (${providerName})`, (it) => {
    ...
    const parent: OrchestrationThread = {
      ...
      session: { ...parentFixture.session!, providerName, providerInstanceId },
    };
    ...
    it.effect("rejects deleted, archived, nested, and unstarted or unsupported parents", () =>
      Effect.gen(function* () {
        for (const candidate of [
          ...
          { ...parent, session: null },
          { ...parent, session: { ...parent.session!, providerName: "opencode" as const } },
        ]) {
    ...
    it.effect("rejects keeping a discarded or incompletely opened side", () =>
      Effect.gen(function* () {
        for (const candidate of [
          ...
          { ...side, session: null },
          { ...side, session: { ...side.session!, providerName: "opencode" as const } },
