...
import {
  ...
  LEGACY_T3_HOME_ENVIRONMENT_VARIABLE,
...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), HOME_DIRECTORY_NAME),
);
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    if (configuredBaseDir !== undefined) {
      output[HOME_ENVIRONMENT_VARIABLE] = resolvedBaseDir;
    } else {
      delete output[HOME_ENVIRONMENT_VARIABLE];
    }
    delete output[LEGACY_T3_HOME_ENVIRONMENT_VARIABLE];

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed service, these
    // leak through and the child server
    ...
    if (!isDesktopMode) {
      ...
      // apps/web/vite.config.ts. Over a remote origin that is invisible: the
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        // localhost here is what breaks any non-localhost origin (LAN or another
        // explicitly configured network): the remote browser dials its own machine.
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment[HOME_ENVIRONMENT_VARIABLE]?.trim() || undefined);
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env[HOME_ENVIRONMENT_VARIABLE] ?? (yield* DEFAULT_T3_HOME);
