...
**Desktop** is the primary surface. It's a full Electron app that bundles the server runner and uses the web app as its renderer. The desktop app can also host a server for explicitly configured remote browser connections.
...
- **provider** means the agent runtime or harness T3 Code talks to, such as Codex, Claude, or OpenCode.
...
- **Clients.** Web and desktop (which wraps web and adds Electron shell/IPC). Shared client logic lives in `packages/client-runtime`.
- **Providers.** Codex, Claude, and OpenCode each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
...
- `apps/web` - React/Vite UI. `apps/desktop` wraps it, and `apps/marketing` is the site.
- `packages/contracts` - Effect/Schema contracts plus small derived helpers. No heavy runtime logic.
- `packages/shared` - shared runtime utils, subpath exports, no barrel.
- `packages/client-runtime` - non-visual connection and state code used by web and desktop.
